...
vsg::RegisterWithObjectFactoryProxy<vsgGIS::TileReader> s_Register_TileReader;
...
//  TileDatabase
//
void TileReader::read(vsg::Input& input)
{
    input.readObject("settings", settings);
...
void TileReader::write(vsg::Output& output) const
{
    output.writeObject("settings", settings);
}
            ...
            if (imageTile)
            {
                ...
                if (tile)
                {
                    ...
                    if (local_lod < settings->maxLevel)
                    {
                        auto plod = vsg::PagedLOD::create();
                        plod->setBound(bound);
                        plod->setChild(0, vsg::PagedLOD::Child{settings->lodTransitionScreenHeightRatio, {}}); // external child visible when it's bound occupies more than 1/4 of the height of the window
                        plod->setChild(1, vsg::PagedLOD::Child{0.0, tile});                          // visible always
...
void TileReader::init(vsg::ref_ptr<const vsg::Options> options)
{
